...
def store_passwords(password, user_email, username, url, app_name):
    try:
        ...
        postgres_insert_query = """INSERT INTO accounts (password, user_email, username, url, app_name) VALUES (%s, 
        %s, %s, %s, %s) """
...
def connect():
    try:
        connection = psycopg2.connect(user='postgres', password='jes9665432', host='127.0.0.1',
...
def delete_users(app_name):
    try:
        connection = connect()
        cursor = connection.cursor()
        postgres_delete_query = """ DELETE FROM accounts where app_name = '""" + app_name + "'"
        cursor.execute(postgres_delete_query, (app_name,))
        connection.commit()
